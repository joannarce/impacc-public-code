...
# term_type: "feature"; makes plots for the selected feature for all relevant datasets. term_extract_param_list is not used

# term_type: "featureset"; calculates composite score for the features and makes plots. term_extract_param_list must have (1) "features" vector of features to use, (2) "projection_coef_list" data.frame from the relevant base_obj, (3) "omic_assay_DFlist" list of all omic data frames same as base_obj1$assay.train.

# term_type: "pathway"; calculates composite score for the leading edge features of that pathway from the provided mHG result table. term_extract_param_list must have (1) "base_obj" object for the relevant module, (2) "result_table_list" list containing enrichment results for all datasets for a selected database and direction, (3) "sign" numeric indicating 1 or -1 - the direction of enrichment. These objects are supplied to composite_features_creation function of the object.

# term_type: "other"; makes one box plot and one trajectory plot for the selected feature (term) from the selected dataset, e.g. "N1_CT" from "nasal_viralload_counts". term_extract_param_list must have (1) "omic_assay_DF" data to get the feature from, (2) "direction" a numeric 1 or -1, which will be multiplied with the feature values to flip the y-axis if needed. 
# E.g. (1) term = "N1_CT"; term_extract_param_list = list(omic_assay_DF = tmp_base_obj$others$other_assay.train$nasal_viralload_counts, direction = -1)
# E.g. (2) term = "AUC RBD IgG"; term_extract_param_list = list(omic_assay_DF = tmp_base_obj$others$other_assay.train$serum_rbd_abtiters_counts, direction = 1)
## Examples for different term_types
# pp_feature = make_box_trajectory_plots_general(term = "ACTN1", term_type = "feature", phase = "train", event_date = "admission", merge123 = T)
# pp_pathway = make_box_trajectory_plots_general(term = "Necroptosis", term_type = "pathway", term_extract_param_list = list("base_obj" = base_obj1, "result_table_list" = base_obj1$MODmghRes$kegg_pos_impacc, "sign" = 1), phase = "train", event_date = "admission", merge123 = T)
# pp_featureset = make_box_trajectory_plots_general(term = "ISGs", term_type = "featureset", term_extract_param_list = list("features" = isgs, "projection_coef_list" = base_obj1$projection_coef, "omic_assay_DFlist" = base_obj1$assays.train), phase = "train", event_date = "admission", merge123 = T)
# pp_other = make_box_trajectory_plots_general(term = "N1_CT", term_type = "other", term_extract_param_list = list(omic_assay_DF = tmp_base_obj$others$other_assay.train$nasal_viralload_counts, direction = -1), phase = "train", event_date = "admission", merge123 = T)

make_box_trajectory_plots_general <- function(term, term_type, term_extract_param_list, phase = "train", event_date = "admission", max_date = 40, merge123 = TRUE, my_comparisons1 = list(c(2,3)), base_obj=base_obj1, make_traj_plots = TRUE) {
  
  tmp_base_obj = base_obj$clone()
  
  #tmp_base_obj$clin.train$trajectory_group = tmp_base_obj$clin.train$trajectory_group_5grp
  #tmp_base_obj$clin.test$trajectory_group = tmp_base_obj$clin.test$trajectory_group_5grp
  if(merge123) {
    tmp_base_obj$clin.train$trajectory_group = ifelse( tmp_base_obj$clin.train$trajectory_group %in% 1:3, 1, tmp_base_obj$clin.train$trajectory_group)
    tmp_base_obj$clin.test$trajectory_group = ifelse( tmp_base_obj$clin.test$trajectory_group %in% 1:3, 1, tmp_base_obj$clin.test$trajectory_group)
  }
  my_comparisons1 <<- my_comparisons1
  #feat = "ACTN1"
  #term = feat
  #d_name = "pbmc_transcriptomics"
  expr_list = list()
  functional_pvalue = rep("NA", 6) %>% setNames(omic_short_names)
  functional_sign = rep("NA", 6) %>% setNames(omic_short_names)
  
  if(phase == "train") {
    omic_assay_DFlist = tmp_base_obj$assays.train
  } else {
    omic_assay_DFlist = tmp_base_obj$assays.test
  }
  
  if(term_type == "feature") {
    for(d in omics_used_DR) {
      if(term %in% colnames(omic_assay_DFlist[[d]]) )
        expr_list[[omic_short_names[d]]] = omic_assay_DFlist[[d]][,term,drop=F]
    }
  }
  
  if(term_type == "featureset") {
    omic_assay_DFlist = term_extract_param_list$omic_assay_DFlist
    for(d in omic_short_names) {
      featset = term_extract_param_list$features
      featset = featset[ featset %in% colnames(omic_assay_DFlist[[omics_used_DR[d]]]) ]
      if(length(featset) > 0) {
        expr_list[[d]] = comoposite_features_creation_unit(
          omic_assay_DF = omic_assay_DFlist[[omics_used_DR[d]]],
          projection_coef_DF = term_extract_param_list$projection_coef_list[[d]],
          leading_edges = featset,
          is_vector = T)
      }
    }
  }
  ...
  if(term_type == "pathway") {
    expr_list = term_extract_param_list$base_obj$composite_features_creation(
      result_table_list = term_extract_param_list$result_table_list,
      term = term,
      core_enrichment_name = "core_enrichment", phase = phase)
    for(d in names(expr_list)) {
      tmp_res = term_extract_param_list$result_table_list[[d]] %>% filter(ID == term)
      functional_pvalue[d] = tmp_res$p.adj
      functional_sign[d] = term_extract_param_list$sign
    }
  }
  
  if(term_type == "other") {
    tmp_df = term_extract_param_list$omic_assay_DF
    tmp = tmp_df[ 
      match( rownames(omic_assay_DFlist[[1]]), 
             rownames(tmp_df)), term ]
    
    expr_list = list(as.data.frame(tmp * term_extract_param_list$direction )) %>% setNames(term)
    functional_pvalue[term] = c("NA") %>% setNames(term)
    functional_sign[term] = c("NA") %>% setNames(term)
  }
  
  
  omic_short_names_tmp = omic_short_names
  if(term_type == "other") {
    omic_short_names_tmp = term
  }

  ### Box plot  
  box_plot_list = list()
  for(d in omic_short_names_tmp) {
    if(d %in% names(expr_list)) {
      box_plot_list[[d]] = tmp_base_obj$box_plot(plot_name = term,
                                                 expr_list = expr_list[d], phase = phase)
      if(merge123) {
        box_plot_list[[d]] = box_plot_list[[d]] +
          scale_x_discrete(labels = c("1-3","4","5")) + 
          scale_color_discrete(labels=c("1-3","4","5"), 
                               type = c("#639A21","#BD7D31", "#9C3418"))
      }
    } else {
      box_plot_list[[d]] = ggplot() + theme_void()
    }
  }
  ...
  if(! make_traj_plots) {
    ...
    return(list(boxplots = box_plot_list))
  }
  ### Trajectory plot
  model_DF = tmp_base_obj$trajectory_test(
    expr_list = expr_list,
  ...
  traj_plot_list = list()
  traj_indiv_plot_list = list()
  for(d in omic_short_names_tmp) {
    if(d %in% names(expr_list)) {
      tmp = condensed_trajectory_plotII(self = tmp_base_obj, expr_list = expr_list[d], 
                                                                   functional_pvalue = as.numeric(functional_pvalue) %>% setNames(names(functional_pvalue)),
                                                                   functional_sign = functional_sign,
                                                                   model_DF = model_DF, phase = phase, event_date = event_date, max_date = max_date, cex_all = 15, term = term)
      traj_plot_list[[d]] = tmp$plot_join
      traj_indiv_plot_list[[d]] = tmp$tmp_list
    } else {
      traj_plot_list[[d]] = ggplot() + theme_void()
    }
  }
  
  my_comparisons1 <<- list(c(4,5))
  
  return(list(boxplots = box_plot_list, trajplots = traj_plot_list, traj_indiv_plot_list = traj_indiv_plot_list))
...
enrichment_aggregation1 = function(pos_list, neg_list = NULL, thr = 0.2){
  pathway_names = c()
  max_list = list()
  for(j in 1:length(pos_list)){
    rownames(pos_list[[j]]) = pos_list[[j]]$ID
...
condensed_trajectory_plotII = function(self, expr_list,
                                     model_DF,
                                     functional_pvalue, 
                                     functional_sign,
                                     ...
                                     max_date = 28,
                                     phase = "train",
                                     event_date = "admission", cex_all = 15, term = term){
  if(phase == "train"){
  ...
  return(list(plot_join = plot_join, tmp_list = tmp_list))
}
