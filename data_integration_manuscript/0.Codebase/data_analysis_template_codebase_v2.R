...
prepare_clinical_data <- function(data_env, KEEP_COVID19_POS, FILTER_BY_CORE_ASSAY_COHORT, FILTER_COHORT_FOR_INTEGRATIVE_ANALYSIS, EVENT_DATE_UPPER, VISIT_UPPER, PHASES) {
...
load_IMPACC_datasets <- function(DATA_VERSION, 
  ...
  data_env <- prepare_clinical_data(data_env, KEEP_COVID19_POS, FILTER_BY_CORE_ASSAY_COHORT, FILTER_COHORT_FOR_INTEGRATIVE_ANALYSIS, EVENT_DATE_UPPER, VISIT_UPPER, PHASES)
...
load_IMPACC_datasets_vDataIntegration <- function(DATA_VERSION, 
  ...
  data_env <- load_IMPACC_datasets(DATA_VERSION, KEEP_COVID19_POS, PHASES, ALLOWED_SMPL_STATUS, FILTER_BY_CORE_ASSAY_COHORT, FILTER_COHORT_FOR_INTEGRATIVE_ANALYSIS, EVENT_DATE_UPPER, VISIT_UPPER)
...
mgcv_global = function(inputDF, age_sex = T, endpoints = "trajectory_group"){
  ...
  res_table$adjp.intercept= ifelse(length(res_table$p.intercept) == 1,
                                   res_table$p.intercept,
                                   qvalue::qvalue(res_table$p.intercept, fdr.level = 0.05, pi0 = 1)$qvalues
  )
  res_table$adjp.slope= ifelse(length(res_table$p.slope) == 1,
                               res_table$p.slope,
                               qvalue::qvalue(res_table$p.slope, fdr.level = 0.05, pi0 = 1)$qvalues
  )
  if(age_sex){
    ...
    res_table$adjp.intercept.sex= ifelse(length(res_table$p.intercept.sex) == 1,
                                         res_table$p.intercept.sex,
                                         qvalue::qvalue(res_table$p.intercept.sex, fdr.level = 0.05, pi0 = 1)$qvalues
    )
    res_table$adjp.intercept.age.quantile= ifelse(length(res_table$p.intercept.age.quantile) == 1,
                                                  res_table$p.intercept.age.quantile,
                                                  qvalue::qvalue(res_table$p.intercept.age.quantile, fdr.level = 0.05, pi0 = 1)$qvalues
    )
  }
...
model_loop <- function(inputDF, modelType = "lme", age_sex =TRUE, knots = c(1, 4, 7, 14, 21),
  ...
  inputDF = inputDF[!is.na(inputDF$value), ]
  ...
  if(modelType == "smoothSpline"){
    ...
    lmDF <- inputDF %>%
      ...
      p.intercept = {
        ...
        if(class(fit)[1]!="try-error"){
          result = (anova(fit, type = "marginal")$"p-value")[3] #trajectory_group is position 3
        }else{
          result = NA
        }
      },
      ...
      if(group_trendline_dropout){
        pred <- pred %>%
          left_join(plotDF %>%
                      ...
                      group_by(.data[[endpoint]]) %>% dplyr::summarize(max = max(event_date)), by = c({{endpoint}})) %>%
