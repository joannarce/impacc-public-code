...
source(paste0(codebase_path, "/codebase_integration.R")) 
...
##### PVCA analysis
run_pvca <- function(assay, data_env, preprocessed_matrix, plate_metadata_column,
                     core_site_metadata_column, plot_title) {
...
preprocess_proteomics <- function(assay, data_env, alpha = 0.99){
  ...
  if (assay == "plasma_proteomics_targeted") {
    plate_metadata_column = "Plate.id"
  }
  if (assay == "plasma_proteomics_global_dda") {
    plate_metadata_column = "plate_num"
  }
...
if(! any(duplicated(indiv_supp$participant_id))) {
  data_env$clinical_data <- left_join(data_env$clinical_data, indiv_supp,
                                      by = "participant_id")
}
...
preprocessed_data_list = preprocess_data(data_env, alpha = 0.3, 
                                         random_seed = 2021,
                                         impute_cores = 8)
preprocessed_data = preprocessed_data_list$preprocessed_data


# saveRDS(preprocessed_data, file = paste0(out_path, "/preprocessed_data.rds"))
saveRDS(data_env, file = paste0(out_path, "/data_env.rds"))
```

# MOFA Imputation (Jeremy)
...
# preprocessed_data <- readRDS(paste0(out_path, "/preprocessed_data.rds"))
# data_env <- readRDS(paste0(out_path, "/data_env.rds"))
...
# Prepare data for MOFA imputation
prepared_data_DR = data_prepare_DR(omics_used_DR, rna_keep = NULL, clinical_data = clinical_data, data_env = preprocessed_data_list)
...
organized_data = list()
organized_data[["datasets"]] =  prepared_data_DR_full$train
organized_data[["datasets_imputed"]] = tmp
organized_data[["MOFAobject.trained"]] = MOFAobject.trained

# get event_ids
event_ids <- rownames(organized_data$datasets_imputed[[1]])
...
clinical_data_min <- clinical_data_min[match(event_ids, clinical_data_min$event_id), ]
rownames(clinical_data_min) <- clinical_data_min$event_id

# save to objects
organized_data[["clinical_data"]] <- clinical_data_min
saveRDS(organized_data, paste0(out_path, "/organized_data.rds"))
```
