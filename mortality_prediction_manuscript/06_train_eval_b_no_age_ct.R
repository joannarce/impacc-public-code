...
## No ct and age, for supp analysis
...
here()
output_dir <- here("06_train_eval_b_no_age_ct")
  ...
  for (bootstrap in seq_len(num_bootstraps)) {
    ...
    for (i in seq_len(num_folds)) {
      ...
      for (j in seq_along(feature_list)) {
        ...
        top_features <- top_features[-length(top_features)] # Remove CT
        print(top_features)
        
        top_features <- top_features[-length(top_features)] # Remove age
        print(top_features)
...
write_csv(auc_results_frame, here(output_dir, "no_age_ct_genes_only_filter_complete_auc_results.csv"))

write_csv(best_results_df, here(output_dir, "no_age_ct_genes_only_filter_best_results_df.csv"))
...
nearby_colors <- c("#b48bb3", "#cc00cc",  # Adjusted pinks/purples
                   "#8590c0", "#bca577",  # Adjusted purple and brown
                   "#63a3c2", "#77a880",  # Adjusted blues and greens
                   "#9ecdb1")            # Adjusted green
...
pretty_plot <- ggplot(compare, aes(x = description, y = AUC_value, fill = description)) +
  ...
  scale_fill_manual(values = nearby_colors) +
  my.theme +
  theme(legend.position = "none") +
  geom_hline(yintercept = 0.9, linetype = "dashed", color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

qsave(pretty_plot, here(output_dir, "no_age_ct_filtered_violin_genes_only.qs"))
ggsave(here(output_dir, "no_age_ct_filtered_violin_genes_only.svg"), pretty_plot, width = 4, height = 4)
