# Here I perform glm for nasal counts to evaluate AUC for gene sets on train data for filtered list
...
here()
output_dir <- here("06_train_eval_c_nasal")
...
## READ IN FILES ======
cv.folds.nasal <- read_csv("04_assemble_bucket_folds/nasal_complete_train_folds.csv")
...
two_feature_list <- qread(here(input_dir, "2_nasal_feature_list.qs"))
three_feature_list <- qread(here(input_dir, "3_nasal_feature_list.qs"))
four_feature_list <- qread(here(input_dir, "4_nasal_feature_list.qs"))
...
two <- determine_3_cv(nasal_counts, cv.folds.nasal, two_feature_list)
three <- determine_3_cv(nasal_counts, cv.folds.nasal, three_feature_list)
four <- determine_3_cv(nasal_counts, cv.folds.nasal, four_feature_list)
...
auc_results_frame <- do.call("rbind", list(results_two$result_df,
                                           ...
                                           results_four$result_df))

## Extract the best perform feature set for each n,

best_results_df <- do.call("rbind", list(results_two$max_auc_row,
                                         results_three$max_auc_row,
                                         results_four$max_auc_row))

write_csv(auc_results_frame, here(output_dir, "filter_complete_auc_results_nasal.csv"))

write_csv(best_results_df, here(output_dir, "filter_best_results_df_nasal.csv"))
...
best_results_df$description <- factor(best_results_df$j, 
...
qsave(pretty_plot, here(output_dir, "nasal_filtered_violin.qs"))
ggsave(here(output_dir, "nasal_filtered_violin.svg"), width = 4.5, height = 6)




## Gene expression of 3.1compared =======

nasal_complete_metadata_clin <- read_csv("01_pbmc_nasal/nasal_complete_metadata_clin.csv")

# Gene expression of 3.1
gene_3.1 <- three_feature_list[[1]][1:3] # select genes
...
genex <- genex %>%
  as.data.frame(.) %>%
  select(c(gene_3.1, "ENSG00000152463")) %>%
  rownames_to_column(var = "sample_id") %>%
  inner_join(., nasal_complete_metadata_clin, by = "sample_id")

genex <- genex %>% 
  mutate(group = ifelse(trajectory_group == 5, 1, 0))

df <- genex[, c("group", gene_3.1)]
...
## Plot
pretty_plot <- ggplot(plotting_O, aes(x = factor(gene, levels = c("SLC5A5", "CD200R1", "FCER1A")), y = expression, fill = factor(group))) +
  geom_boxplot(position = position_dodge(0.9), outlier.shape = NA) +
  scale_fill_manual(values = c("#6b85cd", "#d2485a"),
                    labels = c("0" = "Survival",
                               "1" = "Mortality")) +
  labs(x = "Gene", y = "Normalized expression", fill = "") +
  my.theme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(here(output_dir, "nasal_box_plot_genex_all_data.svg"), height = 5, width = 5.5)
