# Here I perform LASSO regression for pbmc + CT counts, filtering for DEG genes 
...
here()
output_dir <- here("05c_complete_LASSO_deg_filter")
...
## READ IN FILES ======
cv.folds.pbmc <- read_csv(here(input_dir, "pbmc_complete_train_folds.csv"))
...
prepare_counts_folds <- function(counts, meta_folds, fold, numFeatures) {
  ...
  lambda_index <- which(mod$df == numFeatures)[1]
...
## Collect Data =====
n_size <- c(2, 3, 4, 5, 6, 8, 10) # Various gene sizes
for (n in n_size)
{
  print(paste("Processing gene size...", n))
  feature_length =  n + 2
  print(paste("The number of features are: ", feature_length))
  ct_age_fold_results_pbmc <- run_all_folds(pbmc_counts, cv.folds.pbmc, feature_length)
  ...
  for (i in seq_along(ct_age_fold_results_pbmc$results)) {
    # Extract the coef data frame 
    coef_df <- ct_age_fold_results_pbmc$results[[i]]$coef
  ...
  pbmc_output <- data.frame(Gene_name = character(), fold = numeric(), stringsAsFactors = FALSE)
  ...
  for (i in seq_along(gene_names_per_fold))
  {
    ...
    pbmc_output <- rbind(pbmc_output, fold_df)
  }
  
  ## EXPORT ====
  print("Exporting...")
  gene_names_df <- do.call(rbind, lapply(gene_names_per_fold, function(x) data.frame(Gene = x)))
  write_csv(gene_names_df, here(output_dir, paste(n, "pbmc_gene_names_per_fold.csv", sep = "_")))
  write_csv(pbmc_output, here(output_dir, paste(n, "pbmc_geneid_output.csv", sep = "_")))
  qsave(ct_age_fold_results_pbmc, here(output_dir, paste(n, "pbmc_output.qs", sep = "_")))
  qsave(gene_names_per_fold, here(output_dir, paste(n, "pbmc_feature_list.qs", sep = "_")))
