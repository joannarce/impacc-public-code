# Here, I perform DE analysis on PBMC+CT train samples to extract only
...
here()
output_dir <- here("04a_pbmc_ct_train_extract_deg_thresh")
...
## READ IN METADATA ======
pbmc_complete_metadata_train <- read_csv(here(input_dir, "pbmc_complete_train_folds.csv"))
...
de_and_list <- function(counts, metadata) {
  ...
  write_csv(analyze_top, here(output_dir, "de_results_train_only_pbmc_CT.csv"))
  ...
  qsave(geneListThreshold, here(output_dir, "gene_list_threshold.qs"))
  
  
}

de_and_list(pbmc_counts, pbmc_complete_metadata_train)
